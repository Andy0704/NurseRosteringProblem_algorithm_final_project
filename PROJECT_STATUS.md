...
2026-06-20 — EVAL-段3: 60/60 batch (EVAL-段0–段2-pre) analyzed — evaluation/analyze_results.py builds DataFrame, joins organiser-validated best-known costs, renders P1–P5 to results/batch_2026-06-20/plots/; references/eval_results_2026-06-20.md written. 22/22 tests still pass. Key finding: "full" mode (SA) regresses cost vs. milp/fo on 5/14 production datasets (worst: n100w4 +123% absolute), not a strict improvement — see Known Issues
...
| evaluation/ (run_ablation.py, run_batch.py, analyze_results.py) | ✅ Complete (EVAL-段0–段3) | mode-gated milp/fo/full ablation runner + fault-tolerant batch wrapper + pandas/matplotlib analysis; 60/60 (instance,mode) jobs completed 2026-06-20, 9h27m, 0 failures; results in results/batch_2026-06-20/, writeup in references/eval_results_2026-06-20.md |
...
- [2026-06-21] research: EVAL-段3 follow-up diagnostic found F&O parameter-propagation bug invalidating F&O ablation column on 11/14 datasets. Bug + fix logged; deferred. MILP and Full mode results unaffected.
- [2026-06-20] research: EVAL-段3 — analysis + plots + writeup for the 60-job overnight batch (results/batch_2026-06-20/, jobs run in EVAL-段0–段2-pre, not previously logged here). All 60 JSON files validated. evaluation/analyze_results.py (pandas/matplotlib, installed via `pip install --user --break-system-packages` — not previously present despite task assumption) builds one DataFrame, joins the 14-dataset organiser-validated best-known cost table (eval_design_notes.md §3) by dataset, computes gap_pct (NaN for the 3 testdatasets, no public benchmark). Found and fixed a real plotting bug: matplotlib's boxplot renders n=1 boxes as just a median line, which defaults to a fixed color regardless of facecolor — added explicit medianprops per mode so P1's single-instance datasets show the correct mode color instead of all appearing orange. Five plots (P1-P5) + P6 markdown table written to references/eval_results_2026-06-20.md. Key data-backed findings: (1) two negative gap_pct values (n040w4 -15.0%, n050w4 -9.5%) are an evaluator-coverage artifact (S2 CS2a/b, S5 not implemented) not real outperformance of the competition's validated best; (2) "full" mode (SA) regresses cost vs. milp/fo on 5/14 production datasets, worst case n100w4 3560→7950 (+123%, traced to one week's per-week cost going 10→4740) — see Known Issues; (3) fo cost equals milp cost on 11/14 production datasets — F&O had almost no effect at free_count=2 in this run; (4) only n120w8 is genuinely time-budget-constrained (flat 310s/week CBC limit used for all 14 datasets, not the size-scaled Ceschia formula) — 7/8 weeks hit the cap exactly; (5) H3 (hard constraint) clean on all 60 runs. 22/22 tests still pass (no source code touched, per task scope).
...
- [x] EVAL-段0–段3: full evaluation matrix design, ablation runner, overnight 60-job batch, analysis+plots+writeup all complete (2026-06-20). Remaining work is slide writing only.
...
- **[Documented trade-off — S7\* look-ahead, W-10 段1B]** S7\* improves n021w4's per-week INRC-II total by -35.1% (370→240) but increases the global S7 working-weekend excess penalty from 30→210, a net regression of +12.5% (400→450) on the full pipeline. Reproducible via `python3 run_4week_full_pipeline.py --instance n021w4`. Mechanism: each week's MILP sees only its own proportional weekend budget, with no visibility into which other nurses are also near their caps — pushing weekend-avoidance onto whichever nurse has slack *this week* concentrates excess onto fewer nurses by the final week instead of distributing it. Classified as scenario (b) "target met but spill" (per-week target satisfied, cost spills to the global term) — see `references/lookahead_design_notes.md` § W-10 段1B. Not a bug; mirrors S10\*'s analogous regression on n012w8 in 段1A. No action planned — documented for the presentation's honest-tradeoff slide.
...
- **[OPEN — SA stage regresses cost on a substantial minority of instances, EVAL-段3]** Comparing `full` (MILP+F&O+SA) vs. `milp`-only across the 14 production datasets in results/batch_2026-06-20/: 5 of 14 datasets get *worse* after SA, 6 improve, 3 unchanged. Worst case `n100w4`: 3560→7950 (+123%), traced to a single week going from per-week cost 10 to 4740 while the other three weeks stayed flat/improved slightly (reproducible: `python3 -c "import json; print([w['total'] for w in json.load(open('results/batch_2026-06-20/n100w4_0_1-1-0-8_full.json'))[0]['per_week']])"` vs. the `_milp.json` counterpart). Suspected mechanism (not yet verified against source): the per-week SA optimizes local week cost via delta evaluation with no visibility into global S6/S7 or the carry-forward effect of its own moves on neighboring weeks — consistent with Late Acceptance's by-design tolerance for backsliding, but the magnitude here (10→4740 in one week) is large enough to warrant checking whether this is normal LA variance or a delta-evaluation drift (Rule 11). Not investigated further — out of scope for EVAL-段3 (analysis only, no source changes). See references/eval_results_2026-06-20.md "Key findings" for the full breakdown.

- **[Open — F&O sub-problem build() parameter-propagation bug]** Discovered 2026-06-21 via EVAL-段3 follow-up diagnostic. In milp_model.py fix_and_optimize(), the self.build() rebuild call does not forward (is_final_week, cur_week, num_weeks) from the main per-week context. Defaults (True, 1, 1) are used, so F&O sub-problems solve a different objective (no look-ahead terms) than the main solve. Effect: F&O acceptance ~100% rejection on non-final weeks of multi-week instances. Fo column in EVAL-段3 results invalidated. Fix: one-line plumbing change + 14-dataset re-run. Deferred to post-2026-06-24 presentation.
