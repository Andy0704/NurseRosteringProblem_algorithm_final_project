...
| inner_heuristic/src/heuristic.cpp | ✅ Consec cost homologous + SA unfrozen | SA+LA; consec-work/off cost identical to penalty_evaluator (SD-1/2/3 fixed). Hard coverage reject (return 999999) replaced with M_COVER big-M soft penalty (M_COVER≈3×T0, Knust 2019 p0=0.05) in deltaTwoWaySwap/deltaRandomDayOff; best_sched gated on totalH2Units==0. SHIFT_OFF_REQ_W 5→10 (S4 now homologous). |
| inner_heuristic/build/nrp_heuristic | ✅ Built | recompiled 2026-06-13 |
| tests/ (all) | ✅ 19/19 PASS | test_pipeline.py (7) + test_sa_carryin.py (10, deterministic carry-in) + test_sa_identity.py (2: 800 random schedules per-item S2/S3/S4/forbidden <1e-6, + test_sa_h2_feasible_no_bigm_leak) |
